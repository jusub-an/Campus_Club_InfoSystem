			...
			<c:choose>
				...
				<c:when test="${not empty sessionScope.user_email}">
    				<span class="login-user-text me-3 d-none d-sm-inline">
        				<strong>${sessionScope.user_email}</strong>님
    				</span>
    				<a href="${pageContext.request.contextPath}/user/logout" class="btn btn-login-action btn-logout-custom btn-sm">
        				로그아웃
    				</a>
				</c:when>
				...
				<%-- 2. 로그아웃 상태 --%>
				<c:otherwise>
    				<a href="${pageContext.request.contextPath}/user/login" class="btn btn-login-action btn-login-custom btn-sm">
       					 로그인
    				</a>
				</c:otherwise>
			</c:choose>
